// ═══════════════════════════════════════════════════════
// NEO4J AURA - ANLAMLI İLİŞKİLER
// Document bilgisi sadece node property'lerinde
// Toplam Batch: 8
// ═══════════════════════════════════════════════════════

// 📊 OLUŞTURULACAK İLİŞKİLER:
// • REGULATES: Regulation → diğer kurallar
// • PAID_TO: PaymentRule → Entity
// • TRIGGERS: Condition → Prohibition
// • REQUIRES: Condition → Obligation
// • DETERMINES_PAYMENT: BillingRule → FinancialRule
// • HAS_REQUIREMENT: Process → Requirement
// • FOLLOWED_BY: Anlamlı sıralı ilişkiler
// • REFERENCED_IN: ReferenceDocument → Rules
...
MATCH (c:Condition {extraction_index: 10}),
      (p:Prohibition {extraction_index: 10})
CREATE (c)-[:TRIGGERS]->(p);

MATCH (c:Condition {extraction_index: 8}),
      (p:Prohibition {extraction_index: 8})
CREATE (c)-[:TRIGGERS]->(p);

MATCH (c:Condition {extraction_index: 4}),
      (p:Prohibition {extraction_index: 4})
CREATE (c)-[:TRIGGERS]->(p);

MATCH (c:Condition {extraction_index: 3}),
      (p:Prohibition {extraction_index: 3})
CREATE (c)-[:TRIGGERS]->(p);

MATCH (c:Condition {extraction_index: 6}),
      (o:Obligation {extraction_index: 6})
CREATE (c)-[:REQUIRES]->(o);

MATCH (br:BillingRule {extraction_index: 10}),
      (fr:FinancialRule {extraction_index: 11})
CREATE (br)-[:DETERMINES_PAYMENT]->(fr);

MATCH (e1:Regulation {extraction_index: 7}),
      (e2:PaymentRule {extraction_index: 7})
CREATE (e1)-[:FOLLOWED_BY]->(e2);

MATCH (ref:ReferenceDocument {extraction_index: 14}),
      (rule:BillingRule {extraction_index: 15})
CREATE (ref)-[:REFERENCED_IN]->(rule);
...
MATCH (ref:ReferenceDocument {extraction_index: 5}),
...
MATCH (ref:ReferenceDocument {extraction_index: 4}),
...
MATCH (ref:ReferenceDocument {extraction_index: 12}),
