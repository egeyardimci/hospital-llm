...
class QAService:
    ...
    @staticmethod
    def add_qa_pair(query, answer):
        collection = GLOBAL_MONGO_DB_CLIENT.get_queries_collection()
        result = collection.insert_one({"query": query, "answer": answer})
    ...
    @staticmethod
    def update_qa_pair(_id, query, answer):
        collection = GLOBAL_MONGO_DB_CLIENT.get_queries_collection()
        collection.update_one({"_id": ObjectId(_id)}, {"$set": {"query": query, "answer": answer}})