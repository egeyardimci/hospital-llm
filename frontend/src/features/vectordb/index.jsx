...
import {
  ...
  loadVectorDB,
  setSelectedVectorDB,
} from '../../store/slices/vectordbSlice';
import { useAppSelector } from '../../hooks/useAppSelector';
import { toast } from '../../utils/toast';

// Create New Vector DB Component
const CreateVectorDB = () => {
  const embeddingModels = useAppSelector(
    (state) => state.vectorDBs.embeddingModels
  );
...
const LoadVectorDB = () => {
  const dispatch = useAppDispatch();
  const selectedDB = useAppSelector(
    (state) => state.vectorDBs.selectedVectorDB
  );
    ...
    <div className="flex flex-col items-center space-y-6">
      ...
      <div className="w-full max-w-md space-y-4">
        <div>
          ...
          <Select
            theme={customSelectTheme}
            options={vectorDbs.map((db) => ({
              value: `${db.name}_${db.chunk_size}_${db.chunk_overlap}`,
              label: `${db.name}_${db.chunk_size}_${db.chunk_overlap}`,
            }))}
    ...
    if (selectedOption === 'load') {
      return (
        <>
          ...
          <LoadVectorDB selectedDb={selectedDb} setSelectedDb={setSelectedDb} />
        </>
      );
    }
  };
